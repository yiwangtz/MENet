...
class Eval_thread():
    ...
    def run(self):
        ...
        max_e,mean_e = self.Eval_Emeasure_my()
        Res['MaxEm'] = max_e
        Res['MeanEm'] = mean_e
        #Res['Em'] = Em
        ...
        torch.save(
            ...
            os.path.join(self.output_dir, 'detail',
                         self.method + '_' + self.dataset + '.pth'))
    ...
    def Eval_Emeasure_my(self):
        print('eval[EMeasure_my]:{} dataset with {} method.'.format(
            self.dataset, self.method))
        avg_e, img_num = 0.0, 0.0
        values = []
        with torch.no_grad():
            ...
            Em_max = np.max(values)
            Em_mean = np.mean(values)

            return Em_max,Em_mean
